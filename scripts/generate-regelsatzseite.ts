    ...
    abschnitt("matrix", "K-mal-S-Matrix",
      tabelle(["Dimension", "K1", "K2", "K3"],
        Object.keys(dim).map((name) => [`<b>${esc((dim[name] as { label: string }).label)}</b>`,
          `<code>${esc(JSON.stringify(dim[name].K1))}</code>`,
          `<code>${esc(JSON.stringify(dim[name].K2))}</code>`,
          `<code>${esc(JSON.stringify(dim[name].K3))}</code>`]))),

    abschnitt("traceability", "Traceability-Tiefe",
      `<p>Knoten: ${trace.knoten.map((k) => `<code>${esc(k)}</code>`).join(" → ")}</p>` +
      tabelle(["K-Stufe", "Pflichttiefe"], Object.entries(trace.stufen).map(([k, v]) => [`<b>${esc(k)}</b>`, `<code>${esc(JSON.stringify(v))}</code>`]))),
